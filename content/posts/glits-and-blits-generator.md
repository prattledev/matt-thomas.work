...
**BLITS** - Black & Lane's Identification Tones for Surround - is the EBU Tech 3304 equivalent for 5.1 surround. It runs sequential 600 ms identification bursts, one per speaker position, using specific frequencies assigned to each channel, across a full 13.4-second cycle. It is used to verify that a 5.1 path is correctly routed end to end - that the left surround is arriving at the left surround, that the LFE is reaching the subwoofer, and that nothing has been swapped or dropped in the chain.
